...
# constant
config = load_config('KCF_config.yml')
print(config)
NUM_SECTOR = config['num_sector']
FLT_EPSILON = config['flt_epsilon']

print(type(NUM_SECTOR))
print(type(FLT_EPSILON))
...
@cc.export('func2', '(f4[:], f4[:,:], i8[:,:,:], i8[:], f4[:,:], i8,i8,i8,i8,i8,i8,i8)')
def func2(mapp, r, alfa, nearest, w, k, height, width, sizeX, sizeY, p, stringSize):
                    ...
                    if (i * k + ii > 0 and i * k + ii < (height - 1) and j * k + jj > 0 and j * k + jj < (width - 1)):
                        ...
                        if (j + nearest[jj] >= 0 and j + nearest[jj] <= (sizeX - 1)):
                            ...
                            mapp[i * stringSize + (j + nearest[jj]) * p + alfa[
                                ...
                                jj, 1]
                        if (i + nearest[ii] >= 0 and i + nearest[ii] <= (sizeY - 1) and j + nearest[jj] >= 0 and j +
                                nearest[jj] <= (sizeX - 1)):
    ...
    for i in range(sizeY):
        for j in range(sizeX):
            ...
            for jj in range(xp):  # 9
                newData[int(pos2 + 2*xp + jj)] = np.sum(mappmap[pos1 + jj : pos1 + jj + yp*xp : xp])*ny
            for ii in range(yp):  # 4
                newData[int(pos2 + 3*xp + ii)] = np.sum(mappmap[pos1 + yp*xp + ii*xp*2 : pos1 + yp*xp + ii*xp*2 + 2*xp]) * nx
