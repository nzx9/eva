...
def tracker_task(video_id, frame_no, bbox):
    ...
    logger.info('frame_no is: {}'.format(frame_no))

    low = frame_no + 1
    last_frame = (frame_no//settings.TRACKER_SIZE + 1)*settings.TRACKER_SIZE
    logger.info('last_frame is: {}'.format(last_frame))
    num_images = hdf5_file['img'].shape[0]
    high = min(last_frame + 1, num_images)
    logger.info('high is: {}'.format(high))
    logger.info('low is: {}'.format(low))
    t0 = time()
    for i in range(low, high):
        ...
        if ok:
            ...
            break
    t1 = time()
    logger.info('Time it took: {}'.format(t1-t0))
    return coordinates
